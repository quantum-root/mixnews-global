...
def get_news(category):
    news_list = []
    urls = {
        "Top Stories": "https://www.ntv.com.tr/son-dakika",
        "Economy": "https://www.ntv.com.tr/ekonomi",
        "Sport": "https://www.ntv.com.tr/spor"
    }
    try:
        res = requests.get(urls[category], headers=SAFE_HEADERS, timeout=10)
        soup = BeautifulSoup(res.text, "html.parser")
        
        for h in soup.find_all(["h2", "h3", "p"], class_=lambda x: x and "card-text" in x or True):
            text = h.get_text(strip=True)
            link = h.find_parent("a") or h.find("a")
            
            if text and len(text) > 30 and link:
                href = link['href']
                if href.startswith("/"): 
                    href = f"https://www.ntv.com.tr{href}"
